...
namespace ProlappApi.Controllers
{
    ...
    public class NotaCreditoController : ApiController
    {
        ...
        public HttpResponseMessage GetNotaCreditoID(int id)
        {
            ...
            query = @"Select * from NotaCredito where IdFactura =" + id + " and Serie='407292'";
            using (var con = new SqlConnection(ConfigurationManager.ConnectionStrings["Prolapp"].ConnectionString))
            using (var cmd = new SqlCommand(query, con))
            using (var da = new SqlDataAdapter(cmd))
            {
...
values (" + nc.IdCliente + "," + nc.IdFactura + ", '" + nc.Serie + "','" + nc.Folio + "', '" + nc.Tipo + "', '" + time.ToLocalTime().ToString(format) + "', '" + nc.LugarDeExpedicion + "', '" + nc.Certificado + "', '" + nc.NumeroDeCertificado + "', '" + nc.UUID + "', '" + nc.UsoDelCFDI + "' , '" + nc.Subtotal + "','" + nc.Descuento +
...
"', '" + nc.RFCdelPAC + "', '" + nc.Observaciones + "', '" + time2.ToLocalTime().ToString(format) + "', '" + nc.OrdenDeCompra + "', '" + nc.TipoDeCambio + "', '" + time3.ToLocalTime().ToString(format) + "', '" + nc.CondicionesDePago + "', '" + nc.Vendedor + "', '" + nc.Estatus + "', '" + nc.Ver + "', '" + nc.Usuario + "','" +
...
values ("+d.IdNotaCredito+", '"+d.ClaveProducto+"', '"+d.Producto+"', '"+d.Unidad+"', '"+d.ClaveSat+"', '"+d.PrecioUnitario+"', '"+d.Cantidad+"', '"+d.Importe+"', '"+d.Observaciones+"', '"+d.TextoExtra+"', '"+d.PrecioUnitarioDlls+"', '"+d.ImporteDlls+"','"+d.ImporteIVA+"', '"+d.ImporteIVADlls+"');";
        ...
        public string PutCancel(int id)
        {
            try
            {
                ...
                string query = @"update NotaCredito set Estatus='Cancelada' where UUID=" + id;
