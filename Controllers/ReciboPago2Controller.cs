    ...
    public class ReciboPago2Controller : ApiController
    {
        ...
        public string PutCancelarPagoCFDI(int id)
        {
            try
            {
                ...
                DataTable table = new DataTable();
                string query = @"update PagoCFDI2 set saldo=convert(float,Cantidad)+convert(float,saldo), cantidad=0, NoParcialidad=0 where IdReciboPago=" + id + "";
                ...
                query = @"update factura2 set Estatus='Timbrada' where id = (select IdFactura from PagoCFDI2 where IdReciboPago" + id + ")";
